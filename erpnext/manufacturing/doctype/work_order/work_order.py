...
class WorkOrder(Document):
	...
	def validate_work_order(self):
		# Check if the Work Order has a Production Plan but no Material Request
		if self.production_plan and not self.material_request:
			# Fetch the Production Plan using frappe.get_doc
			production_plan = frappe.get_doc("Production Plan", self.production_plan)
			
			if production_plan:
				# Get the material requests associated with the Production Plan
				material_requests = production_plan.get('material_requests')
				
				if material_requests and len(material_requests) > 0:
				# Set the first material request to the Work Order's material_request field
					self.material_request = material_requests[0].get('material_request')
					# self.material_request = id[0].get('id')
				else:
				# If no material requests are found in the Production Plan
					print('No Material Request found in the selected Production Plan.')
			else:
				# If no Production Plan is found with the selected name
				print('No Production Plan found with the selected name.')

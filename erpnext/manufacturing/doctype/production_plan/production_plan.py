...
from frappe.utils import cstr, flt, cint, nowdate, add_days, comma_and, now_datetime, ceil, getdate, add_days, nowtime
...
class ProductionPlan(Document):
	...
	def before_save(self):
		self.fetch_delete_data()
		self.fetch_lookup_table()
	def on_submit(self):

		self.make_work_order()

	def on_cancel(self):
		    # Cancel associated Work Orders
		if self.material_requests:
			for material_request in self.material_requests:
				work_orders = frappe.get_all('Work Order', filters={
				'production_plan': self.name,
				'material_request': material_request.material_request,
				'docstatus': 1  # Only cancel submitted Work Orders
				})

				for work_order in work_orders:
					wo_doc = frappe.get_doc('Work Order', work_order.name)
					wo_doc.cancel()
					frappe.msgprint(_("Work Order {0} has been canceled.").format(work_order.name))

		# Perform any additional cancellation logic if needed
		super(ProductionPlan, self).on_cancel()
...
# ---------------------------------- modify --------------------------------------------
	...
	def fetch_delete_data(self):
		if self.new_doc:
			self.delet = []
			# Fetch the linked record from the 'New' DocType
			new_doc = frappe.get_doc('new', self.new_doc)
			# frappe.msgprint(f"Fetched New Doc: {new_doc}")
			...
			# Clear the existing rows in the start table in Production Plan

				for row in new_doc.start:
					self.append('start', {
						'item_name': row.item_name,
						'description': row.description,
						'days_no' : row.days_no
...
# ------------------------------------lookup table------------------------------------
	def fetch_lookup_table(self):
		if self.lookup_item:
			self.lookup = []
			# Fetch the linked record from the 'New' DocType
			lookup_item = frappe.get_doc('lookup item', self.lookup_item)
			# frappe.msgprint(f"Fetched New Doc: {new_doc}")
...
# ----------------------------------- modify ------skip items---------------------
	def get_items_to_skip(self):
		# Fetch the items to skip from the delet table for the current Production Plan
		skip = []
		
		# Assuming 'delet' is a child table in 'Production Plan'
		# Fetch items from the current Production Plan's 'delet' table
		if hasattr(self, 'delet') and self.delet:
			skip = [row.item_name for row in self.delet]
...
# ----------------------------------- modify ------start items---------------------
	...
	# 		start = [{'item_name': row.item_name, 'dayss': row.dayss} for row in self.start]
	...
	def get_items_to_start(self):
		...
		# Assuming 'start' is a child table in 'Production Plan'
		# Fetch items from the current Production Plan's 'start' table
		if hasattr(self, 'start') and self.start:
			for row in self.start:
				item_name = row.get("item_name")
				days_no = row.get("days_no")
				print(f"Item Name: {item_name}, Days: {days_no}")  # Debug print statement
				start.append({
				'item_name': item_name,
				'days_no': days_no
				})
...
# ----------------------------------------- modify -------------------------------------
	...
	def make_work_order_for_sub_assembly_items(self, item):
		...
		# Fetch the sub-assembly items
		get_sub_assembly_items(item.get("bom_no"), bom_data, item.get("qty"))

		# Get the items to skip
		items_to_skip = self.get_items_to_skip()

		for key, data in bom_data.items():
			# Skip Work Orders for specific items listed in delet_item
			if data.get('item_name') in items_to_skip:
				continue
...
# ------------------------------------------------------------------------
	...
	def create_work_order(self, item):
		...
		wo.update(item)
		   ...
		   # Fetch the schedule_date from the Production Plan
		schedule_date = frappe.db.get_value('Production Plan', self.name, 'schedule_date')

		# Update planned_start_date with schedule_date
		if schedule_date:
			current_time = datetime.strptime(nowtime(), '%H:%M:%S.%f').time()
			wo.planned_start_date = datetime.combine(getdate(schedule_date), current_time)
			print(f"Planned Start Date set to Schedule Date: {schedule_date}")
		else:
			print("No Schedule Date found in Production Plan")

		# Modify - Check if the item_name is in the start earlier items list and get the corresponding days
		start_earlier_items = self.get_items_to_start()
		days_delay = 0

		print("Start Earlier Items:")
		for start_item in start_earlier_items:
			print(f"Item Name: {start_item['item_name']}, Days: {start_item['days_no']}")  # Debug print statement
			if item.get("item_name") == start_item['item_name']:
				days_delay = start_item.get("days_no", 0)
				break

		# Update Planned Start Date based on the days_delay
		if days_delay is not None and days_delay > 0:
			if wo.planned_start_date:
				# Get the existing date and time
				original_date = getdate(wo.planned_start_date)
				original_time = get_datetime(wo.planned_start_date).time()

				# Calculate the new date based on days_delay
				new_date = add_days(original_date, -days_delay)
				
				# Combine the new date with the original time
				wo.planned_start_date = datetime.combine(new_date, original_time)
				print(f"Planned Start Date adjusted by {days_delay} days: {wo.planned_start_date}")
			else:
				print("No planned_start_date found to adjust.")
		else:
			print(f"No days_delay provided or days_delay is {days_delay}. No adjustment needed.")

		# # Set the warehouse if specified
		# if item.get("warehouse"):
		# 	wo.fg_warehouse = item.get("warehouse")
		
		# # Set default warehouse if none specified
		# if not wo.fg_warehouse:
		# 	wo.fg_warehouse = warehouse.get('fg_warehouse')

    # Fetch the `wip_warehouse` and `fg_warehouse` from `Production Plan Item` and `lookup` table
		item_code = item.get('production_item')
		if not item_code:
			print("Error: item_code is None or empty")
			print(f"Received item data: {item}")
			return None

		print(f"Fetching WIP Warehouse for item_code: {item_code}")

		wip_warehouse = frappe.db.get_value('Production Plan Item', 
						{'parent': self.name, 'item_code': item_code}, 
						'wip_warehouse')

		if wip_warehouse:
			wo.wip_warehouse = wip_warehouse
		else:
			print(f"No WIP Warehouse found for item_code {item_code}. Checking lookup table...")
			wip_warehouse = frappe.db.get_value('lookup', 
						{'production_item': item_code}, 
						'wip_warehouse')

			if wip_warehouse:
				wo.wip_warehouse = wip_warehouse
				print(f"Found WIP Warehouse in lookup table for item_code {item_code}.")
			else:
				print(f"No WIP Warehouse found for item_code {item_code} in both Production Plan Item and lookup table.")
				frappe.throw(f"No WIP Warehouse found for item_code {item_code}.")

		# fg_warehouse = frappe.db.get_value('Production Plan Item', 
		# 				{'parent': self.name, 'item_code': item_code}, 
		# 				'warehouse')
		...
		# else:
			print(f"No Target Warehouse found for item_code ({item_code}). Checking lookup table...")
			fg_warehouse = frappe.db.get_value('lookup', 
						{'production_item': item_code}, 
						'fg_warehouse')

			if fg_warehouse:
				wo.fg_warehouse = fg_warehouse
				print(f"Found Target Warehouse in lookup table for item_code {item_code}.")
			else:
				print(f"No Target Warehouse found for item_code {item_code} in both Production Plan Item and lookup table.")
				frappe.throw(f"No Target Warehouse found for item_code {item_code}.")


		wo.set_work_order_operations()
		wo.validate_work_order()
		# Fetch the workstation for the specific item_code in the po_items table within the Production Plan
		item_code = item.get('production_item')
		...
		# Fetch the workstation linked to the item_code in the Production Plan's po_items table
		po_items_workstation = frappe.db.get_value('Production Plan Item', 
									{'parent': self.name, 'item_code': item_code}, 
									'workstation')

		# Set Workstation in Work Order operations if found
		if po_items_workstation:
			for operation in wo.operations:
				operation.workstation = po_items_workstation
			print(f"Workstation {po_items_workstation} set for item {item_code}")
		else:
			print(f"No workstation found for item_code {item_code}")



		# Insert the Work Order document
		try:
			wo.insert()
			# time.sleep(3)	
			wo.save()
			...
			# Return the name of the newly created Work Order
			return wo.name
		except OverProductionError:
			# Handle the case where over-production is not allowed
			print(f"Error: Over-production not allowed for Work Order with item {item.get('item_name')}")
			return None

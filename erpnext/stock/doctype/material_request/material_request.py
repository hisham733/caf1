...
from erpnext.stock.doctype.material_request.pp import create_production_plans
...
class MaterialRequest(BuyingController):
	...
	def validate_id(self):
		if not self.id:
			last_id = frappe.db.get_value('Material Request', {}, 'id', order_by='id desc')
			if last_id:
				new_id = ('000' + str(int(last_id) + 1))[-3:]
			else:
				new_id = '001'
			self.id = new_id
...
# ---------------------------------------------------------------------------------------------------------------
	...
	def on_submit(self):
		...
		#  modify -Ensure there are items in the Material Request before proceeding
		if not self.get('items'):
			frappe.throw(_("No items found in the Material Request"))

		# Create Production Plan for the Material Request
		create_production_plans(self.name)
...
# -------------------------------------------------------------------------
	...
	# -------------------------modify---------------------
	def update_qty_or_size(self):
		# if item.size_qty:
		for item in self.items:
			# Check if item_code starts with 'recipe'
			if item.item_code and item.item_code.startswith('Recipe'):
				if item.batch_size and item.size_qty:
					# Calculate total size and update the field
					item.qty = item.batch_size * item.size_qty
					item.stock_qty = item.qty
					# frappe.msgprint(f"Calculated Total Size: {item.qty}")
				# elif item.total_size and item.size:
				# 	# Calculate quantity based on total size and size
				# 	item.qty = item.total_size / item.batch_size
				# 	frappe.msgprint(f"Calculated Quantity: {item.qty}")
				else:
					frappe.throw(f" Batch Size, not provided for item: {item.item_code}")
		# else:
            # # Optionally log if the item does not start with 'Recipe' and is skipped
            # # frappe.msgprint(f"Item {item.item_code} does not start with 'Recipe' and was skipped.")
           	# 	pass
	# -------------------------modify---------------------
...
def create_production_plan(material_request):
    ...
    production_plan.append('material_requests', {
            ...
            'id': material_request_doc.id,  # Data field (assuming item_code is used as ID)
    ...
    # Link the Production Plan to the Material Request
    material_request_doc.production_plan = production_plan.name

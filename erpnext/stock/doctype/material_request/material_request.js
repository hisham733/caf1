...
frappe.ui.form.on('Material Request', {
	...
	get_item_data: function(frm, item) {
		...
		// Fetch BOM quantity if bom_no is set
		if (item.bom_no) {
			frappe.call({
				...
				callback: function(r) {
				if(r.message) {
					let bom_qty = r.message.quantity; // Assuming 'quantity' is the field name in BOM
					frappe.model.set_value(item.doctype, item.name, "size_qty", bom_qty);
				} else {
					frappe.msgprint(__('No BOM found with the selected number.'));
				}
				}
			});
		}
    ...
    refresh: function (frm) {
        if (frm.doc.docstatus === 1 && !frm.doc.production_plan) {
            frm.add_custom_button(__('Create Production Plan'), function () {
                frappe.call({
                    method: 'erpnext.stock.doctype.material_request.material_request.create_production_plan',
                    args: {
                        material_request: frm.doc.name
                    },
                    callback: function (r) {
                        if (!r.exc) {
                            frm.reload_doc();
                            frappe.msgprint(__('Production Plan Created'));
                        }
                    }
                });
            });
            ...
            frappe.call({
                ...
                callback: function(response) {
                    if (response.message && response.message.id) {
                        let last_id = parseInt(response.message.id);
                        frm.set_value('id', ('000' + (last_id + 1)).slice(-3));
                    } else {
                        frm.set_value('id', '001');
                    }
                }
            });
        }

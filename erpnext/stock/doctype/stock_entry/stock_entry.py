...
class StockEntry(StockController):
	...
	def get_transfered_raw_materials(self):
		transferred_materials = frappe.db.sql("""
			select
				sed.item_name, sed.original_item, sed.item_code, sum(sed.qty) as qty, sed.t_warehouse as warehouse,
				sed.description, sed.stock_uom, sed.expense_account, sed.cost_center
			from `tabStock Entry` se
			join `tabStock Entry Detail` sed on se.name = sed.parent
			where
				se.docstatus=1 and se.purpose='Material Transfer for Manufacture'

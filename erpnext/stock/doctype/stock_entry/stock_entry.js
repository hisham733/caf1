...
frappe.ui.form.on('Stock Entry', {
    refresh: function(frm) {
        if (frm.doc.docstatus === 1 && frm.doc.stock_entry_type === 'Manufacture') { // Ensure only submitted Stock Entry has the button
            frm.add_custom_button(__('Create Balance Qty'), function() {
                // Create a new Balance Qty without saving it automatically
                frappe.model.with_doctype('Balance QTY', function() {
                    var new_doc = frappe.model.get_new_doc('Balance QTY');
                    new_doc.stock_entry = frm.doc.name; // Pass Stock Entry reference

                    // Add items from Stock Entry to Balance QTY
                    new_doc.items = frm.doc.items.map(item => {
                        return {
                            'item_code': item.item_code,
                            'qty': item.qty,
                            'uom': item.uom,
                            's_warehouse': item.s_warehouse,
                            't_warehouse': item.t_warehouse, // Source or target warehouse
                            'conversion_factor': item.conversion_factor,
                            'transfer_qty': item.transfer_qty,
                        };
                    });

                    // Open the new Balance QTY form in the frontend without saving
                    frappe.set_route('Form', 'Balance QTY', new_doc.name);
                });
            }, __('Create')); // Button under 'Create' menu
        }
    }
});
